...
void read_inputs() {
  freq_val = mozziAnalogRead(FREQ_PIN);
  v_oct_val = mozziAnalogRead(V_OCT_PIN);
...
void FM_setFreqs(Q16n16 freq) {
  // mod_freq = ((freq >> 8) * ((p1_pot_val + p1_cv_val) / 2));
  // deviation = ((mod_freq >> 16) * (1 + p2_pot_val + p2_cv_val));
  // osc1.setFreq_Q16n16(freq);
  // osc2.setFreq_Q16n16(mod_freq);

  mod_freq = (freq >> 1) * ((p1_pot_val + p1_cv_val) >> 5);       //新版倍频算法 0.5-16倍频
  deviation = ((mod_freq >> 16) * (1 + p2_pot_val + p2_cv_val));  //op2amount
  osc1.setFreq_Q16n16(freq);                                      //给主波形设置频率
  osc2.setFreq_Q16n16(mod_freq);
}
...
void ADD_setFreqs() {
  ...
  //OSC frequency knob
  ADD_freq1 = freq_val / 8;
  ADD_freqv1 = ADD_freq1 * pow(2, (pgm_read_float(&(voctpow[v_oct_val]))));  // V/oct apply
...
void updateControl() {
  ...
  if (p1_pot_val >= 1020) {  //inv knob max
    ...
    if (mode == 0) {
      knob_freq = freq_val;
      // FM_setFreqs((2270658 + knob_freq * 5000) * pow(2, (pgm_read_float(&(voctpow[v_oct_val])))));//old
      FM_setFreqs((2143658 + knob_freq * 5200) * pow(2, (pgm_read_float(&(voctpow[v_oct_val])))));  //新的基音 c0 32.7hz
    } else if (mode == 2) {
      ADD_setFreqs();
    } else {
      CHORD_setFreqs();
...
AudioOutput_t updateAudio() {
  if (mode == 0) {
    // return MonoOutput::fromNBit(16, (osc1.phMod(Q15n16(deviation * osc2.next() >> 8)) / 2) * (gain_val / 2));
    return MonoOutput::fromNBit(16, osc1.phMod(deviation * osc2.next() >> 8) << 8);
  } else if (mode == 2) {
    return MonoOutput::fromNBit(16, (osc1.next() * (pgm_read_byte(&(ADD_gain_table[0][ADD_gain]))) / 512 + osc2.next() * (pgm_read_byte(&(ADD_gain_table[1][ADD_gain]))) / 512 + osc3.next() * (pgm_read_byte(&(ADD_gain_table[2][ADD_gain]))) / 512 + osc4.next() * (pgm_read_byte(&(ADD_gain_table[3][ADD_gain]))) / 512 + osc5.next() * (pgm_read_byte(&(ADD_gain_table[4][ADD_gain]))) / 512 + osc6.next() * (pgm_read_byte(&(ADD_gain_table[5][ADD_gain]))) / 512 + osc7.next() * (pgm_read_byte(&(ADD_gain_table[6][ADD_gain]))) / 512 + osc8.next() * (pgm_read_byte(&(ADD_gain_table[7][ADD_gain]))) / 512) * (gain_val / 4));
  } else {
    return MonoOutput::fromNBit(16, ((osc1.next() / 8 + osc2.next() / 8 + osc3.next() / 8 + osc4.next() / 8 + osc5.next() / 8 * inv_aply5) * gain_val));
  }
}
