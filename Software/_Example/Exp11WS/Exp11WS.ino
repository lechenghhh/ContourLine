...
void updateControl() {
  ...
  smoothed_freq = kSmoothFreq.next(target_freq * 4);  // temporarily scale up target_freq to get better int smoothing at low values 暂时放大target_freq以在低值时获得更好的int平滑
  aSin.setFreq(33);                                   // then scale it back down after it's smoothed平滑后再缩小

  waveshapeLV = mozziAnalogRead(4) >> 3;
}

int updateAudio() {
  char asig0 = aSin.next();  // sine wave source
  //用切比雪夫多项式曲线进行波形整形时，使两个信号渐弱以显示幅度的影响
  //将信号偏移128，以适应波形表查找的0-255范围
  byte asig1 = (byte)128 + ((asig0 * ((byte)128 + waveshapeLV)) >> 8);
