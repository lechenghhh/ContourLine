...
//6knob synth  knob:a0-a5 ledpin:d13 voct:d6 cv:d7 gate:d12 miditrs:tx rx
...
Oscil< SIN512_NUM_CELLS, CONTROL_RATE > lfo_one(SIN512_DATA),
  lfo_two(SIN512_DATA);
//Amplitude envelope
ADSR< CONTROL_RATE, CONTROL_RATE> adsr_envelope;
//Filter Envelope
ADSR< CONTROL_RATE, CONTROL_RATE > adsr_filter;
...
// 读取四个电位计，并将结果与之前的结果进行比较发现。返回一个2d数组，其中第一个位置是读取电位计的位置第二个是第一个位置的值，如果满足以下条件，则布尔值为True,读取的值与上次读取的值不同。
...
void updateControl() {
  ...
  if (digitalRead(12) != tmpD12 && tmpD12 == 0) {  //d13按钮可以用来测试
    tmpD12 = 1;
    handle_note_on(0, 33, 111);
  }
  if (digitalRead(12) != tmpD12 && tmpD12 == 1) {
    tmpD12 = 0;
    handle_note_off(0, 33, 111);
  }
}
