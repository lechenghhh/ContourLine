...
Oscil<256, AUDIO_RATE> aPha2(HALFSINWINDOW512_DATA);
Oscil<TRIANGLE_DIST_SQUARED_2048_NUM_CELLS, AUDIO_RATE> aHSin3(TRIANGLE_DIST_SQUARED_2048_DATA);
Oscil<100, AUDIO_RATE> aSqu4(SQUARE_NO_ALIAS512_DATA);  //SQUARE_NO_ALIAS512_NUM_CELLS
Oscil<SAW256_NUM_CELLS, AUDIO_RATE> aSaw5(SAW256_DATA);
...
void updateControl() {
  ...
  aCheb7.setFreq_Q16n16(toneFreq);  //给主波形设置频率、音高
  aSqu4.setFreq_Q16n16(toneFreq);   //给主波形设置频率、音高
  aSaw5.setFreq_Q16n16(toneFreq);   //给主波形设置频率、音高
  aPha2.setFreq_Q16n16(toneFreq);   //给主波形设置频率、音高
  aHSin3.setFreq_Q16n16(toneFreq);  //给主波形设置频率、音高
  aCheb6.setFreq_Q16n16(toneFreq);  //给主波形设置频率、音高
  aNos.setFreq_Q16n16(toneFreq);    //给主波形设置频率、音高
...
AudioOutput_t updateAudio() {
  ...
  switch (Wave) {
    default:
      ...
      break;
    case 1:
      asig = MonoOutput::from8Bit(aPha2.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 2:
      asig = MonoOutput::from8Bit(aHSin3.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 3:
      asig = MonoOutput::from8Bit(aSqu4.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 4:
      asig = MonoOutput::from8Bit(aSaw5.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 5:
      asig = MonoOutput::from8Bit(aCheb6.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 6:
      asig = MonoOutput::from8Bit(aCheb7.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      break;
    case 7:
      // asig = MonoOutput::from8Bit(aNos.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
      return MonoOutput::from8Bit(aNos.phMod(modulation));  // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
  }
  //波形渐变算法
  int wtasig = 0;
